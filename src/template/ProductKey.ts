export default class ProductKey {
    ...
    static readonly zs_banner_text_time = 0;//: number;                 //广点通文字延时移动时间（单位：毫秒）
    static readonly zs_banner_move_time = 500;//                        //广点通文字上移动画时间长度（单位：毫秒）
    static readonly zs_banner_horizontal_enable = 0;//: boolean;        //广点通文字左右移动开关（0关，1开）
    static readonly zs_revive_type = 0;//: number;                      //游戏复活方式（0不复活，1狂点复活，2视频复活，3分享复活，4普通复活）
    static readonly zs_revive_click_num = 0;//: number;                 //游戏狂点复活次数（-1不限制，0使用视频复活，N次后使用视频复活）
    static readonly zs_revive_video_num = 0;//: number;                 //游戏视频复活次数（-1不限制，0使用分享复活，N次后使用分享复活，没视频了使用分享复活）
    static readonly zs_revive_share_num = 0;//: number;                 //游戏分享复活次数（-1不限制，0使用普通复活，N次后使用普通复活）
    static readonly zs_continue_auto_share = 0;//: boolean;
    static readonly zs_full_screen_jump = 0;//: boolean;           //增加全屏导出位开关
    static readonly zs_revive_countdown = 10;//: number;               //复活倒计时
    static readonly zs_jump_style = 1;//: number;                      //结算页样式
